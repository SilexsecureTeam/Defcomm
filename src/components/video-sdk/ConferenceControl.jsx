...
import {
    ...
    FaVideo, FaCog, FaDesktop, FaStopCircle, FaHandPaper, FaRegHandPaper, FaCommentDots
} from "react-icons/fa";
...
const ConferenceControl = ({ handleLeaveMeeting, handleScreenShare, isScreenSharing, me }) => {
    ...
    // Show red dot on new incoming message if chat is closed
    useEffect(() => {
        if (!showChatModal && chatMessages.length > 0) {
            const latest = chatMessages[chatMessages.length - 1];
            if (latest.message.id !== myId) {
                setHasNewMessage(true);
            }
        }
    }, [chatMessages, showChatModal, myId]);

    // Reset red dot when chat modal is opened
    useEffect(() => {
        if (showChatModal) {
    ...
    return (
        <>
            <div className="sticky mt-auto bottom-0 bg-black/70 flex justify-center items-center gap-8 text-2xl py-4 z-10">
                ...
                <button onClick={handleLeaveMeeting} className="bg-red-500 text-white w-12 h-12 rounded-full flex items-center justify-center hover:bg-red-700">
                    <FaPhone />
                </button>
