...
function DashTabs() {
    ...
    return (
      ...
      {tabOptions?.map((item, idx) => (
        <motion.li key={idx} onClick={() =>{dispatch({...item}); navigate(item?.route)}} className={`${state?.route !== item?.route ? "bg-oliveDark/70" : "bg-white"} cursor-pointer p-3 rounded-xl flex-shrink-0`} whileHover={{ scale: 1.1 }}>
