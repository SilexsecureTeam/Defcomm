...
const FolderFiles = ({ data, loading, error, addFile }) => {
  ...
  const toggleImportant = (index) => {
    setFiles((prevFiles) => {
      ...
      updatedFiles[index] = {
        ...updatedFiles[index],
        important: !updatedFiles[index].important,
      };
      return updatedFiles;
    });
  };
            ...
            <tbody>
              {data?.map((file, index) => (
                <motion.tr
                  ...
                  onClick={() =>
                    navigate(
                      `/dashboard/file-view/${btoa(import.meta.env.VITE_FILE)}`
                    )
                  }
                ...
                >
                  ...
                  <td className="p-3">{file?.file_size}</td>
                  <td className="p-3">{file?.file_ext}</td>
